...
def _determine_filename(filepath, camera_id, timestamp=datetime.now(), event_id="0", frame="0"):
    return config.Development.TARGET_DIR + filepath\
...
def _send_socket_msg(camera_id, picture_type, timestamp, filename, event_id="0"):
    UDP_IP = config.Development.MOTION_SOCKET_ADDR
    UDP_PORT = config.Development.MOTION_SOCKET_PORT
...
def create_snapshot_frame(camera_id="1"):
    ...
    filename = _determine_filename(config.Development.SNAPSHOT_FILENAME, camera_id, timestamp)
...
def create_motion_frame(camera_id="1"):
    ...
    filename = _determine_filename(config.Development.MOTION_FILENAME, camera_id, timestamp, event_id)
    _create_image_file(filename)
