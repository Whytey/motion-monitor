...
class MotionDetectConfig():
  def __init__(self):
    ...
    self.movie_filename = 'camera%t/%Y%m%d/%v/%Y%m%d-%H%M%S'
...
def create_other_keep_list():
  regexes = []

  # For motion detection
  regexes.append(re.compile('/data/motion/motion/.*'))

  # For timelapse
  regexes.append(re.compile('/data/motion/timelapse/.*'))
...
class TimelapseConfig():
  ...
  def __init__(self):
    ...
    self.target_dir = '/data/motion/snapshots'
    # Extract the following from the config
    self.snapshot_filename = 'camera%t/%Y/%m/%d/%H/%M/%S-snapshot.jpg'
...
def main():
  ...
  try:
    ...
    regexes.extend(create_other_keep_list())
